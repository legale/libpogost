...
static void hmac_parts(u8 out[32], const u8 *key, size_t key_len,
                     ...
                     const char *label, const u8 *seed, size_t seed_len)
{
  ...
  hmac_parts(a, secret, secret_len, (const u8 *)label, label_len,
             seed, seed_len, NULL, 0);
  while (out_len) {
    hmac_parts(block, secret, secret_len, a, sizeof(a),
               (const u8 *)label, label_len, seed, seed_len);
