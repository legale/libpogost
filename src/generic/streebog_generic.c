...
/*
 * Streebog hash function as specified by GOST R 34.11-2012.
 *
 * Copyright (c) 2013 Alexey Degtyarev <alexey@renatasystems.org>
 * Copyright (c) 2018 Vitaly Chikunov <vt@altlinux.org>
 *
 * Adapted from the Linux generic implementation for libpogost.
 */
...
_Static_assert(sizeof(struct streebog_state) <= sizeof(struct streebog_ctx),
               "STREEBOG_CTX_SIZE is too small");
...
void streebog_init(struct streebog_ctx *pub, unsigned int bits)
{
  ...
  for (i = 0; i < 8; i++) {
    if (ctx->digest_size == STREEBOG256_DIGEST_SIZE)
